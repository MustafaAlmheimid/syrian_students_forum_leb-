...
app.get('/api/question-replies', async (req, res) => {
  try {
    ...
    const replies = await sql`
      SELECT * FROM question_replies WHERE question_id = ${question_id} ORDER BY created_at ASC
    `;
    res.json(replies);
  } catch (err) {
    console.error(err);
...
app.get('/api/admin/users', async (req, res) => {
  try {
    if (!(await checkAdmin(req))) return res.status(403).json({ error: 'Admin only' });
    const users = await sql`
      SELECT id, email, first_name, last_name, birthday, university, major, phone, role, created_at
...
app.get('/api/profile/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const users = await sql`
      SELECT id, email, first_name, last_name, birthday, university, major, phone, role
...
app.put('/api/profile', async (req, res) => {
  try {
    const { id, first_name, last_name, birthday, university, major, phone } = req.body;
    const [updatedUser] = await sql`
      UPDATE users
      SET first_name = ${first_name}, last_name = ${last_name}, birthday = ${birthday},
          university = ${university}, major = ${major}, phone = ${phone}
      WHERE id = ${id}
      RETURNING *
    `;
...
app.post('/api/forgot-password', async (req, res) => {
  try {
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; direction: rtl; text-align: right; background: #f9fafb; padding: 40px 20px;">
          <div style="max-width: 600px; margin: auto; background: white; border-radius: 20px; padding: 40px; border: 1px solid #e5e7eb;">
            <div style="text-align: center; margin-bottom: 30px;">
              <h1 style="color: #047857; margin: 0; font-size: 28px;">إعادة تعيين كلمة المرور</h1>
              <p style="color: #6b7280; margin-top: 10px; font-size: 15px;">ملتقى الطلاب السوريين في لبنان</p>
            </div>
            <p style="color: #374151; font-size: 16px; line-height: 1.8;">تلقينا طلباً لإعادة تعيين كلمة المرور الخاصة بحسابك.</p>
            <p style="color: #374151; font-size: 16px; line-height: 1.8;">اضغط على الزر التالي لإكمال العملية:</p>
            <div style="text-align:center; margin:40px 0;">
              <a href="${resetLink}" style="background:#047857; color:white; text-decoration:none; padding:14px 28px; border-radius:12px; font-size:16px; font-weight:bold; display:inline-block;">إعادة تعيين كلمة المرور</a>
            </div>
            <p style="color:#6b7280; font-size:14px; line-height:1.8;">إذا لم تقم بطلب إعادة تعيين كلمة المرور، يمكنك تجاهل هذه الرسالة بأمان.</p>
            <hr style="border:none; border-top:1px solid #e5e7eb; margin:30px 0;">
            <p style="text-align:center; color:#9ca3af; font-size:13px;">© ملتقى الطلاب السوريين في لبنان</p>
          </div>
        </div>
      `
    });
...
app.post('/api/reset-password', async (req, res) => {
  try {
    ...
    const users = await sql`SELECT * FROM users WHERE reset_token = ${token}`;
    const user = users[0];

    if (!user) return res.status(400).json({ error: 'الرابط غير صالح' });
    if (Date.now() > user.reset_token_expiry) return res.status(400).json({ error: 'انتهت صلاحية الرابط' });
  ...
  try {
    ...
    for (const user of users) {
      await resend.emails.send({
        ...
        subject: title,
        html: `
          <div style="font-family: Arial, sans-serif; background: #f5f7f9; padding: 40px 20px;">
            <div style="max-width: 650px; margin: auto; background: white; border-radius: 20px; overflow: hidden; border: 1px solid #e5e7eb;">
              <div style="background: linear-gradient(135deg, #047857, #065f46); padding: 35px; text-align: center; color: white;">
                <h1 style="margin: 0; font-size: 30px;">ملتقى الطلاب السوريين</h1>
                <p style="margin-top: 10px; opacity: 0.9;">Syrian Students Forum in Lebanon</p>
              </div>
              <div style="padding: 40px;">
                <h2 style="color: #111827; margin-bottom: 25px;">${title}</h2>
                <p style="color: #374151; line-height: 2; font-size: 16px; white-space: pre-line;">مرحباً ${user.first_name || ''}،\n\n${message}</p>
                <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #e5e7eb; color: #6b7280; font-size: 14px;">تم إرسال هذه الرسالة من منصة ملتقى الطلاب السوريين في لبنان.</div>
              </div>
            </div>
          </div>
        `
